...
def train_severity_model(train_fire_df: pd.DataFrame, test_fire_df: pd.DataFrame) -> tuple[RandomForestClassifier, list[str], str, dict[int, float], dict[str, Any]]:
    ...
    model = RandomForestClassifier(n_estimators=500, criterion="log_loss", max_features="sqrt", min_samples_leaf=8, max_samples=0.85, class_weight="balanced_subsample", random_state=42, n_jobs=-1)
